...
Before spawning investigators, inspect the tools and connectors exposed in the current Codex task. Use the available-tools map or tool search when present. Do not scan configuration directories for credentials or unrelated connectors.
...
Local source control is available through Git. Use `gh` only when it is installed, authenticated, and the requested evidence is in scope. For the other categories, classify the available connector by its name, instructions, tools, and resource descriptions. If a connector could fit more than one category, choose the category that matches its primary evidence and record the ambiguity.
...
Launch matching investigators in concurrent waves that fit the available Codex agent slots. One investigator owns one evidence category so its coverage stays easy to audit. Tell every investigator that the task is read-only and that it must not edit files or mutate external systems. Prefer inherited models.
...
Spawn one Codex synthesizer after the investigators finish. Tell it that the task is read-only. Give it access to the evidence pointers needed to spot-check citations, but do not authorize writes or external mutations. Prefer the inherited model.
