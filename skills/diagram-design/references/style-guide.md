...
|---|---|---|---|
| `paper` | Page background, default node fill | `#ffffff` (brand white) | `#333333` |
| `paper-2` | Diagram container bg, secondary fill | `#f7f7f7` (menu hover gray) | `#525252` |
| `ink` | Primary text, primary stroke | `#171717` (near-black) | `#ededed` |
| `muted` | Secondary text, default arrow stroke | `#737373` (neutral-500) | `#cfc6b8` |
| `soft` | Sublabels, boundary labels | `#a3a3a3` | `#8e98ac` |
| `rule` | Hairline borders | `rgba(23,23,23,0.12)` | `rgba(237,237,237,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#d4d4d4` (neutral-300) | `rgba(207,198,184,0.25)` |
| `accent` | Focal / 1–2 max per diagram | `#f4b41b` (A.E.S. amber) | `#f6c915` |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(244,180,27,0.10)` | `rgba(246,201,21,0.12)` |
| `link` | HTTP/API calls, external arrows | `#28ccdf` (A.E.S. cyan) | `#5cd9e6` |

> **Brand palette source:** A.E.S. website (aes.land, light theme). Core brand colors — near-black `#171717`, neutral grays `#737373`/`#f7f7f7`, amber `#f4b41b`, lime `#b6d53c`, cyan `#28ccdf`. `soft` and `rule` are derived from the neutral scale; `link` reuses the brand cyan. Lime is available as an auxiliary series color for charts needing a third hue.
