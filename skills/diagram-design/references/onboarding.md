...
Use the installed-skill location exposed by Codex when available. Otherwise check:

1. `$CODEX_HOME/skills/<skill-name>/` when `CODEX_HOME` is set.
2. `~/.codex/skills/<skill-name>/` for the default personal installation.
3. The active plugin skill roots exposed by the current Codex session.
4. A project-specific path supplied by the repository or the user.
