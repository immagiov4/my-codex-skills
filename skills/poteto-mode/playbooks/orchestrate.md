...
- **Coordinator.** Frames work, writes briefs, owns the human report, and makes judgment calls. Use Codex collaboration tools to spawn and drain agents. Landing, pushing, or merging work still requires user authorization and repository permission. Use `scripts/orch/orch.ts` only when its runtime and state location are available.
- **Sub-coordinator.** Add one only when the current Codex agent-slot limit leaves room and a track genuinely needs its own coordinator. It rolls up child results at wave boundaries and never forwards raw reports.
- **Worker or verifier.** All Codex agents share the workspace. Give each writer a separate worktree or output path. Tell verifiers not to edit. Prefer fewer, broader workers and keep the total active agents within the current slot limit.
...
Create `orchestrate/<project-slug>/` in a task-local scratch or store path that Codex explicitly exposes. Keep it out of the repository unless the user wants it committed. Every file has one writer. Use `bun scripts/orch/orch.ts` only when Bun is available. The TSV and JSON files remain the source of truth.
...
- Check agents with Codex status and wait tools. Do not restart an idle agent merely to poll it. Use the ledger, `units.tsv`, authorized GitHub state, and durable artifacts as supporting evidence.
...
- After a Codex task restart, re-read the standing orders and `units.tsv`, recompute the frontier, inspect durable branches and pull requests when authorized, and respawn only the work that has no completed artifact. Do not assume an old agent is still running unless Codex reports it.
