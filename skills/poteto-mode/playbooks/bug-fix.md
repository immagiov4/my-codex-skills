...
2. Binary-search the cause. Form candidate hypotheses, then rule them out until one survives. Seed them with `$how` over the affected subsystem and `$why` for regression history. Each pass takes the split that removes the most remaining possibilities. When state is unclear, add targeted instrumentation and read it as the code runs. Use the available Codex wait or monitoring mechanism for a long hunt. Confirm the surviving mechanism with runtime evidence before the architecture or adversarial-review pass.
3. Plan the fix. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured bug-fix model (default `gpt-5.6-sol` with `max` reasoning) with a specific scope; review the diff.
