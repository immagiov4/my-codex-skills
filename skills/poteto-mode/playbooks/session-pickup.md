...
1. Locate the prior trail from an artifact, a task transcript Codex explicitly exposes, a task URL, or an authorized branch. Never scan unrelated task or chat directories. Read the summary and latest messages first, then scan back for decisions. Use a subagent to reduce a long transcript when collaboration is authorized.
