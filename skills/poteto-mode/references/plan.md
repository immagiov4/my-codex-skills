...
State your read of scope and constraints in one paragraph. Ask the user only when intent is genuinely ambiguous or a product decision changes the plan. Give concrete options and a recommendation.
...
Delegate independent codebase exploration when the user or active workflow authorizes subagents. Use Codex collaboration tools, prefer inherited models, and respect the available slot limit.
...
- Browser, Electron, or web UI: use `$agent-browser` or an available in-app browser skill.
- CLI or TUI: use the terminal and the project's verification skill.
