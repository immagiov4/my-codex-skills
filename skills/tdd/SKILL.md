...
6. **Fix the bug.** Make the smallest production change that satisfies the intended behavior while preserving nearby contracts.
7. **Rerun the regression test.** Confirm the test now passes.
8. **Run nearby validation.** Run relevant adjacent tests, type checks, lint, or scenario checks when the change has broader risk.
...
- Do not rerun the same passing command on unchanged code for reassurance. Run it again only after a change that can affect the result.
