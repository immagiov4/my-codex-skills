...
At the end of the run, check the log against the active conversation and the task artifacts exposed by Codex. Use a transcript path only when the environment explicitly provides one for this task. Never scan unrelated task or chat directories. Walk the log against what actually happened:
...
Before handing back, spawn an independent read-only subagent when collaboration is available and the workflow authorizes delegation. Give it the audit trail and the task evidence Codex exposes. It flags what the user should pay attention to. This is a risk scan, not a redo of the work.
...
Every reply for a run that produced a trail ends with an "Attention" section. Name the independent reviewer when one ran, then point each flag to a specific row or moment. "No flags" is valid. If no collaboration slot or subagent capability was available, say that the independent pass did not run.
